  ...
  transformRequestData: (
    ...
    if (isShowMessage) {
      if (hasSuccess && (successMessageText || isShowSuccessMessage)) {
        ...
        // 是否显示自定义信息提示
        ElMessage.error({
          message: message || errorMessageText || "操作失败！",
        });
    ...
    switch (code) {
      ...
      case ResultEnum.TIMEOUT:
        ...
        break;
      case 50004:
        // token已过期 刷新token后重新请求
        const backoff = new Promise((resolve) => {
          refreshToken().then(({ accessToken, refreshToken }: any) => {
            let user;
            try {
      ...
      case 502:
      case 503:
        if (isShowErrorMessage === undefined || isShowErrorMessage === true) {
          ElMessage.error({ message: errorMsg });
        }
        break;
    }
  ...
  responseInterceptorsCatch: (error: any) => {
    ...
    try {
      if (code === "ECONNABORTED" && message.indexOf("timeout") !== -1) {
        ElMessage.error({ message: "接口请求超时，请重试!" });
        return;
      }
    ...
    if (!isCancel) {
      // TODO:Error: Failed to execute 'open' on 'XMLHttpRequest': Invalid URL
      // ElMessage.error({
      //   message: error,
      // });
